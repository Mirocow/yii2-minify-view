        ...
        foreach ($css_files as $file) {
          ...
          if (!empty($m[0])) {
            ...
            foreach ($m[0] as $k => $v) {
              ...
              if (preg_match('#^(' . implode('|', $this->schemas) . ')#is',
                $url)) {
                $result[$m[1][$k]] = '\'' . $url . '\'';
              } else {
            ...
            $content = str_replace(array_keys($result), array_values($result),
              $content);
          }
          ...
          if (!file_exists($js_minify_file)) {
            ...
            foreach ($files as $file => $html) {
              ...
              if ($this->obfuscate_js) {
                $code = (new \GK\JavascriptPacker($code, $this->obfuscate_js_encoding, $this->obfuscate_js_fastDecode, $this->obfuscate_js_specialChars))->pack();
              }
    ...
    if (!empty($this->js)) {
      ...
      if ($ready) {
        ...
        if (!file_exists($js_minify_file)) {
          ...
          if ($this->obfuscate_js) {
            $inline_code = (new \GK\JavascriptPacker($inline_code, $this->obfuscate_js_encoding, $this->obfuscate_js_fastDecode, $this->obfuscate_js_specialChars))->pack();
          }
